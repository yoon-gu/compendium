...
\begin{multicols}{2}
    ...
    \label{alg:sar-train}
    \begin{algorithmic}
    \State \textbf{입력:} 데이터 $\x$, 블록 수 $B$, 정방향 노이즈 과정 $q_t(\cdot|\x)$, 모델 $\x_\theta$, 손실 $\mathcal{L}_{\text{BD}}$
    \Repeat
        \State $t_1, \dots, t_B \sim \mathcal{U}[0, 1]$ 표집
        % \State $\x_{t_1}^1, \dots, \x^B_{t_B} \sim q_{t_1}, \dots, q_{t_B}$ 표집
        \State $\forall b \in \{1,...,B\}:$ $\x_{t_b}^b \sim q_{t_b}(\cdot|\x^b)$
        \State $\emptyset, \mathbf{K}^{1:B}, \mathbf{V}^{1:B} \gets \x_\theta(\x)$ \Comment{KV 캐시}
        \State $\forall b \text{: } \x_\text{logit}^b, \emptyset, \emptyset \gets \x_\theta^b(\x_{t_b}^b,  \mathbf{K}^{1:b\text{-}1}, \mathbf{V}^{1:b\text{-}1})$
        \State $\x_\text{logit} \gets \x_\text{logit}^1 \oplus \dots \oplus \x^B_\text{logit}$로 둠
        \State $\nabla_\theta \mathcal{L}_{\text{BD}}(\x_{\text{logit}}; \theta)$로 경사 단계 수행
    ...
    \label{alg:sar-inference}
    \begin{algorithmic}
    \State \textbf{입력:} 블록 수 $B$, 모델 $\x_\theta$, 디퓨전 표집 알고리즘 \textsc{Sample}
    \vspace{3pt}

    \State $\x, \mathbf{K},\mathbf{V} \gets \emptyset$ \Comment{출력 \& KV 캐시}
    \vspace{3pt}
    \For{$b = 1$ to $B$}
