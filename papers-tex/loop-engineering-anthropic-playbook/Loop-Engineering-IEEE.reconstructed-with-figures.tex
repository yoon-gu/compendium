...
\caption{The four-layer stack. Each layer minds something larger than the one below; loop engineering automates the “waiting for you” that the harness leaves behind.}
\end{figure}
...
\caption{The five moves of one turn. Scheduling closes the cycle—it feeds an unfinished turn into the next day’s run. Verification is the move that can say “no.”}
\end{figure}
...
\caption{Generator and evaluator as separate agents. The evaluator carries none of the generator’s self-persuasion, defaults to doubt, and judges behavior by acting rather than just reading.}
\end{figure}
...
\caption{Each anti-pattern is one move skipped. The five failures map one-to-one onto the five moves of a single turn.}
\end{figure}
...
\caption{Stripe’s Minions pipeline. Deterministic gates (blue) and LLM steps (green) interlock; anything rule-bound is kept out of the probabilistic model. Reliability comes from the constraints, not model size.}
\end{figure}
...
\caption{The four costs reinforce one another. Unverified output erodes understanding, which invites surrender, which lets the loop run longer and spend more, which produces more unverified output.}
\end{figure}
