...
폭넓게 적용 가능한 단어 표현을 배우는 일은 수십 년 동안 활발한 연구 주제였으며, 비-신경망 방식~\cite{brown-etal:1992:_class, ando-zhang:2005, blitzer-mcdonald-pereira:2006:_domain}과 신경망 방식~\cite{mikolov-etal:2013, pennington-socher-manning:2014:_glove}을 모두 포함한다. 사전학습된 단어 임베딩은 현대 NLP 시스템의 핵심 구성요소이며, 처음부터 학습된 임베딩에 비해 큰 폭의 향상을 제공한다~\cite{turian-ratinov-bengio:2010:_word_repres}. 단어 임베딩 벡터를 사전학습하는 데에는 좌→우 언어모델링 목적함수가 사용되었고~\cite{minh09}, 좌·우 맥락에서 올바른 단어와 잘못된 단어를 구분하는 목적함수도 사용되었다~\cite{mikolov-etal:2013}.

이러한 접근은 문장 임베딩~\cite{kiros-etal:2015:_skip, logeswaran2018an}이나 단락 임베딩~\cite{le-mikolov:2014:_distr} 같은 더 굵은 단위로도 일반화되었다. 문장 표현을 학습하기 위해 기존 연구는 후보 다음 문장의 순위를 매기는 목적함수~\cite{DBLP:journals/corr/JerniteBS17, logeswaran2018an}, 이전 문장의 표현이 주어졌을 때 다음 문장의 단어를 좌→우로 생성하는 목적함수~\cite{kiros-etal:2015:_skip}, 또는 잡음제거 오토인코더에서 유도된 목적함수~\cite{hill16}를 사용해 왔다.
...
자연어 추론~\cite{conneau-EtAl:2017:EMNLP2017}과 기계 번역~\cite{mccann-etal:2017:_learn_trans}처럼 데이터셋이 큰 지도 과제로부터의 효과적인 전이를 보인 연구도 있다.
컴퓨터 비전 연구도 대규모 사전학습 모델로부터의 전이 학습이 중요함을 입증해 왔으며, ImageNet으로 사전학습된 모델을 파인튜닝하는 것이 효과적인 레시피임이 알려져 있다~\cite{imagenet_cvpr09, yosinski2014transferable}.
