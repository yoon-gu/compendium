...
표~\ref{tab:main_results}에 제시된 결과는, 대부분의 파라미터를 과제 간 공유함에도 불구하고 PRAGMA가 평가된 거의 모든 도메인에서 기존 과제별 베이스라인(baseline)을 일관되게 능가함을 보여준다.
가장 두드러진 향상은 영향이 큰 과제들의 정밀도-재현율(precision-recall) 지표에서 관찰된다 — 신용 평가에서 PR-AUC가 130.2\,\% 증가했고, 커뮤니케이션 인게이지먼트에서 79.4\,\% 증가했다. 이는 PRAGMA가 전통 모델이 어려워하는 저빈도·고가치 신호를 식별하는 데 매우 효과적임을 시사한다.
