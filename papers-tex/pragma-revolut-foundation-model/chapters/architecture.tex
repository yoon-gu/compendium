...
이 모델은 마스킹된 입력 토큰을 복원하는 마스킹 모델링(MLM) 목적함수로, 대규모이면서 다양성이 큰 데이터셋 위에서 학습된다.
...
모든 사이즈 변형은 GELU 활성 함수~\citep{hendrycks2016gaussian}, 사전 정규화(pre-norm) 레이어 정규화~\citep{xiong2020layer}, 0.1의 드롭아웃~\citep{srivastava2014dropout}을 사용한다.
...
단계에 따라 최종 출력은 사전학습 시 MLM 헤드, 파인튜닝 시 분류 헤드, 또는 임베딩 프로브에서 그대로 사용된다.
...
학습 손실은 라벨 스무딩이 적용된 교차 엔트로피이다~\citep{szegedy2016rethinking}.
...
\emph{LoRA 파인튜닝 모드}에서는 모델 가중치(어텐션과 피드포워드 사영)의 작은 일부(약 2--4\,\%)를 Low-Rank Adaptation~\citep{hu2022lora}으로 갱신한다. 이는 사전학습 백본을 거의 그대로 동결시켜 catastrophic forgetting의 위험을 줄여 준다.
