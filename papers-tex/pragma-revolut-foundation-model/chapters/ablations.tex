...
이 과제는 단순한 전환 예측을 넘어, 중단된 신용 신청 사용자를 다시 끌어들이는 데 어떤 메시징 전략이 가장 효과적인지를 식별하는 \emph{최적 처치 선택(optimal treatment selection)}으로 나아간다.
...
구체적으로, 저자들은 이질적 처치 효과(heterogeneous treatment effect)를 추정하는 메타러너(meta-learner) 프레임워크~\citep{kunzel2019metalearners}를 채택하며, 모델이 이탈 직전 이벤트 신호, 프로필 상태, 처치 할당(treatment assignment) 사이의 복잡한 상호작용을 포착하도록 요구한다.
...
표준 PRAGMA 아키텍처에서는 텍스트 값이 다른 모든 표 형식 피처와 함께 임베딩 룩업 테이블(lookup table)을 통해 공동 학습된다(\S\ref{sec:token_embedding} 참조).
모델이 희소하거나(sparse) 잡음이 많거나(noisy) 매우 불규칙한 금융 텍스트(예: 잘린 거래 설명)에 과소적합(underfit)되지 않도록, 저자들은 텍스트 이해를 별도의 사전학습된 텍스트 임베딩 모델(예: Nemotron-1B-v2~\citep{moreira2024nvretriever})로 분리하는 안을 검토한다.
...
둘째, MLM 최적화(\S\ref{sec:training} 참조) 동안 이 텍스트 필드의 정확한 토큰 라벨을 복원하는 대신, 사전학습된 텍스트 인코더가 만들어 내는 연속(continuous) 텍스트 임베딩을 평균제곱오차(mean squared error, MSE)로 복원하도록 PRAGMA를 학습시킨다.
