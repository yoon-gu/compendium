...
- **이벤트가 곧 토큰**이라기보다, "키 1개 + 값 N개 + 시간 1개"로 이벤트를 펼쳐 *내부 위치 임베딩이 필드 안*에서만 의미를 갖게 한 점. 텍스트 LLM의 토큰화를 표 형식 데이터로 옮길 때의 가장 깔끔한 해법 중 하나.
- **시퀀스 패킹 + 동적 배칭** 으로 패딩 오버헤드를 제거해 처리량 2~5배 향상. 운영 환경에서 효율성 차원의 영향이 큰 디테일.
